...
pub async fn insert_data_into_customers_table(&self, customer: Customer) -> Result<Option<Customer>, Error> {
	let mut client = self.db.client.lock().expect("Failed to lock the client");
	let query_result = client.query("SELECT count(*) FROM customers WHERE email = @p1", &[&customer.get_email()]).await.with_context(|| "error execution query")?;
